...
# Detectar se está no Render (usa PostgreSQL) ou local (usa SQLite)
...
def init_db():
    ...
    if USE_POSTGRES:
        ...
        categories = ['Alimentação', 'Transporte', 'Salário', 'Outros', 'Aluguel', 
                     'Cartão', 'Financiamento', 'Impostos', 'Moradia']
...
def dashboard():
    ...
    if USE_POSTGRES:
        # PostgreSQL: usar TO_CHAR ao invés de strftime
        if month:
            cursor.execute("SELECT SUM(amount) as total FROM transactions WHERE type='income' AND TO_CHAR(TO_DATE(date, 'YYYY-MM-DD'), 'YYYY-MM') = %s", (month,))
            income = cursor.fetchone()['total'] or 0
            cursor.execute("SELECT SUM(amount) as total FROM transactions WHERE type='expense' AND TO_CHAR(TO_DATE(date, 'YYYY-MM-DD'), 'YYYY-MM') = %s", (month,))
            expense = cursor.fetchone()['total'] or 0
            cursor.execute('''
                SELECT c.name, SUM(t.amount) as total
                FROM transactions t
                JOIN categories c ON t.category_id = c.id
                WHERE t.type='expense' AND TO_CHAR(TO_DATE(t.date, 'YYYY-MM-DD'), 'YYYY-MM') = %s
                GROUP BY c.name
            ''', (month,))
...
def add_transaction():
    if request.method == 'POST':
        ...
        for i in range(repetitions):
            ...
            if USE_POSTGRES:
                cursor.execute('INSERT INTO transactions (type, amount, category_id, date, description) VALUES (%s, %s, %s, %s, %s)',
                             (type_, amount, category_id, date_formatted, description))
            else:
                cursor.execute('INSERT INTO transactions (type, amount, category_id, date, description) VALUES (?, ?, ?, ?, ?)',
                             (type_, amount, category_id, date_formatted, description))
...
def edit_transaction(transaction_id):
    ...
    if request.method == 'POST':
        ...
        if USE_POSTGRES:
            cursor.execute('UPDATE transactions SET type=%s, amount=%s, category_id=%s, date=%s, description=%s WHERE id=%s',
                         (type_, amount, category_id, date_str, description, transaction_id))
            cursor.close()
        else:
            cursor.execute('UPDATE transactions SET type=?, amount=?, category_id=?, date=?, description=? WHERE id=?',
                         (type_, amount, category_id, date_str, description, transaction_id))
        conn.commit()
        conn.close()
        return redirect(url_for('dashboard'))
